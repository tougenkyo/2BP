...
旧2Bはwebp・webmやサイトデザインの仕様についていけなかった所や長時間使い続けるとメモリ逼迫やフリーズが頻繁する問題があったのと<br>
丁度ClaudeAIのトークンが余っていたのでこれらを解消すべく新しく作り直すことにしました
...
| **`run_2bp_loop.bat`** | 自動再起動付きの起動用です。万が一クラッシュした場合に、自動で再起動しながら使い続けたい場合に使用してください。再起動のために黒い窓が残りますが、閉じても 2BP は終了しません（自動再起動だけが止まります）。 |

初回起動時に `python` が見つからない、または起動に失敗する場合は、`setup_2bp.bat` を実行してから再度お試しください。

2BP は黒いコンソールウィンドウを出さずに動きます。動作ログはメニューバーの「ヘルプ」→「ログウインドウ」で見られます。「設定」→「その他」→「ログ」で、起動時にログウインドウを開くか・ログをテキストファイル（`logs\console` フォルダ）に保存するかを別々に選べます。
...
また本ソフト内でメニューバーの「ヘルプ」→「アップデートの確認」からアップデート可能です
...
| `futaba2b_log.py` | ログウインドウ・ログのテキスト出力 |
...
設定ファイル・キャッシュ・ログ（`futaba2b_settings.json` など）は、2BP の実行フォルダ内に自動生成されます。
