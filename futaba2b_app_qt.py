    ...
    def _on_tab_changed(self, idx: int):
        ...
        if self._prev_tab_idx >= 0:
            ...
            if isinstance(old_w, ThreadView):
                ...
                QTimer.singleShot(0, lambda v=_old_view: v.page().runJavaScript('try{extractPostsPopup("");}catch(e){}'))
        ...
        if saved:
            js_q = saved.replace("\\", "\\\\").replace('"', '\\"')
            w._view.page().runJavaScript(f'try{{extractPostsPopup("{js_q}");}}catch(e){{}}')
...
class ThreadView(QWidget):
    ...
    def _do_extract(self, query: str = ""):
        """テキストにマッチするレスをポップアップパネルで表示（空のとき閉じる）"""
        q = (query if isinstance(query, str) else self._search_edit.text()).strip()
        js_q = q.replace("\\", "\\\\").replace('"', '\\"')
        self._view.page().runJavaScript(f'try{{extractPostsPopup("{js_q}");}}catch(e){{}}')
